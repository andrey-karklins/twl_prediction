...
for (dataset, G) in datasets:
    apply_fourier_transform(dataset, filename=f'results/plots/{G.name}_{seconds_to_human_readable(G.graph["delta_t"])}_fft.png')
    # Baseline
    baseline_score = model_no_fit(dataset, BaseModel())
    ...
    write_top_sd_results_to_file(sd_results, baseline_score, filename=f'results/{G.name}_{seconds_to_human_readable(G.graph["delta_t"])}_sd.txt', top_n = 5)
    write_top_scd_results_to_file(scd_results, baseline_score, filename=f'results/{G.name}_{seconds_to_human_readable(G.graph["delta_t"])}_scd.txt', top_n = 5)